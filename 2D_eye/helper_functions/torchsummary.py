...
def summary(model, input_size, args, batch_size=-1, device="cuda"):
    '''
    ...
    x = [torch.rand(2, *in_size).cuda(args.gpu).type(dtype)
    ...
    for layer in summary:
        ...
        if 'Conv' in layer:
            ...
            mac=in_channel*out_channel*np.prod(summary[layer]['filter'])*np.prod(summary[layer]["output_shape"][2:])
        ...
        line_new = "{:>20} {:>20} {:>10} {:>10} {:>8}".format(layer, str(summary[layer]["output_shape"]),str(filter), mac, "{0:,}".format(summary[layer]["nb_params"]))
